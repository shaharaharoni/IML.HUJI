...
if __name__ == '__main__':
    ...
    feature_evaluation(X, y, 'correlations')
    ...
    for p in range(10, 101):
        ...
        standard_dev_loss += [np.std(loss)]

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=[i / 100 for i in range(10, 101)], y=mean_loss))
    fig.add_trace(
        go.Scatter(x=[i / 100 for i in range(10, 101)], y=np.asarray(mean_loss) + (2 * np.asarray(standard_dev_loss)),
